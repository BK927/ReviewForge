...
`pnpm install`, `pnpm dev`, `pnpm build`, `pnpm test`는 필요한 네이티브 의존성을 자동으로 준비합니다.
문제가 생기면 아래 명령으로 런타임별 재준비를 수동 실행할 수 있습니다.

```bash
# Node 테스트 런타임용 better-sqlite3 재준비
pnpm run native:prepare:node

# Electron 앱 런타임용 Electron/better-sqlite3 재준비
pnpm run native:prepare:electron
```
