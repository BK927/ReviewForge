...
- 카드 목록 API와 UI는 전체 연결 근거 수만 보지 않고 `match`, `partial`, `reject`, `unverified` 근거 수를 분리해 표시한다. verifier가 없는 deterministic 카드가 검증 통과처럼 보이지 않도록 `미검증 연결`으로 낮춰 표현한다.
- 리포트 화면에는 `강점 활용 메모`를 추가해 유지할 재미, 홍보 소재, 유저 표현 후보, 과장 주의를 분리한다. 이 메모는 외부 마케팅 문구를 자동 확정하지 않고, 원문 대조가 필요한 후보로만 다룬다.
- 카드 상세에는 `판단 리스크`를 추가해 검증 통과 부족, 부분/제외 근거, 추천 리뷰 속 불만, 언어 편중, 강점 카드의 반대 근거를 바로 보이게 한다.
- `GET /api/issues`는 `priority_factors`를 계산해 도달 리뷰 수, verdict mix, 언어 분포, polarity fit, 우선순위 이유를 제공한다. 이는 확률이 아니라 카드 정렬과 회의 판단을 설명하기 위한 기술적 근거다.
- `segment_factors`는 evidence의 플레이타임 구간, 추천/비추천 구성, 언어 편중을 요약한다. 같은 카드도 초반 플레이어 불만인지, 장기 플레이어 반복 피로인지, 추천 리뷰 속 아쉬움인지 구분하기 위한 힌트다.
